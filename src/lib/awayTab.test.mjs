...
test('on the web there is nothing to open and nothing to close', async () => {
  ...
  assert.equal(await openAway('https://photos.google.com/picker', { get }), false)
  assert.equal(await closeAway({ get }), false)
})
...
test('in a wrapper it opens the picker itself, so it can be put away again', async () => {
  ...
  assert.equal(await openAway('https://photos.google.com/picker', { get }), true)
  assert.equal(opened, 'https://photos.google.com/picker')
  assert.equal(await closeAway({ get }), true)
...
test('a picker that will not open is reported, not thrown', async () => {
  ...
  assert.equal(await openAway('https://x', { get }), false)
})
...
test('no address, no sheet', async () => {
  ...
  assert.equal(await openAway('', { get }), false)
