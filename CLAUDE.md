...
- **Idea floated, not yet built**: a private/unshared notes layer under Journal — free-text or dictated reflections (e.g. medication/logistics notes) that never appear in Share links, with optional AI summarization. Needs a schema + UI decision (separate table? a `private_note` column with share-view explicitly excluding it?) before building.
