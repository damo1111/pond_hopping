...
- **Reported, not yet investigated**: Phrases tab (a) doesn't have phrases for every trip (only Korean + Cantonese seeded — Thailand/Singapore/Malaysia/NZ/Voyage have none) and (b) doesn't react to the trip filter — toggling `selectedTrip` doesn't change what's shown.
