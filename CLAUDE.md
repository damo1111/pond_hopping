...
- **Fixed: header/nav briefly flickering visible during the boot splash** (9 Jul, David sent a screen recording): `.boot` had its own 0.9s fade-**in** on mount, so the app underneath (already fully rendered, just visually covered) showed through while the overlay was still translucent. Separately — and this was the actual cause of the "appears, vanishes, reappears" pattern in the recording — `main.jsx`'s service-worker `controllerchange` handler fired an unconditional immediate `window.location.reload()`, which could land mid-boot-animation (very likely mid-session given how many deploys shipped) and looked exactly like a stuck freeze followed by a second boot cycle. `main.jsx` now just flags the pending update; `App.jsx` only reloads once its own `booting` state has settled.
- **Fixed again: reload could still land right after boot, flashing loaded→splash** (9 Jul, David sent an iOS recording, "I don't like this jitter" — turned out to be splash → loaded Home tab → back to splash): deferring the reload until boot settles stopped it interrupting the boot *animation*, but with deploys landing this often an update can just as easily arrive the moment boot finishes, which still force-reloaded instantly. `App.jsx` now only reloads while the tab is actually `document.visibilityState === 'hidden'` — immediately if it's already backgrounded, otherwise on the next `visibilitychange` to hidden — so a fresh version just waits quietly for the next time the app is actually reopened instead of yanking the user away from what they're looking at. Likely to keep happening/reappearing in different forms all session, precisely because of how many deploys are shipping today.
- **Trip cover images can now be set from the gallery, not just Google Photos** (9 Jul, David: "need longer term to be able to pick one from the gallery" — after asking how to set a New Orleans hero image, which has no Google Photos album at all): the lightbox in Photos has a "☆ set as trip cover" button that upserts `photo_cache` with the open photo's own Supabase Storage url — same table/mechanism the old scrape-album flow used, just writable directly by the anon client (already permissive). Surfaced a real bug while building this: cover rendering always assumed a Google Photos url and appended its `=w###-h###-c` resize suffix, which 404s on a Storage url. New `coverUrl()` in `imgTransform.js` picks the Storage transform endpoint or the Google suffix based on the url itself.
- **`trips.subtitle` surfaced on the World-tab cards** (9 Jul, after David edited subtitles directly in the Supabase table editor and asked to confirm that was fine — it is, and always will be for simple content edits): it was already fetched via `trip_meta` but only ever rendered in `ShareView`. Small addition to `WorldTab.jsx`'s `.wt-card`.
- **Map tab improvements, round 1** (9 Jul, "go forward with map tab improvements"): new **Photos** filter chip isolates the photo-GPS trail on its own instead of it only ever bundling into "All". The photo trail + its popups are now tinted with the selected trip's own `tripColor()` instead of a flat gold — New Orleans also got its own accent (deep burgundy, `#8B3A42`) added to `tripColors.js` since it was silently falling back to the generic default everywhere (route-strip, day-scrubber, hero band, and now Map all pick it up). Photo popups gained a "→ view in journal" jump link (date-matched), same as pins/runs already had. New day-scrubber (mood/day chips, floats above the bottom nav when a trip is selected) flies the Leaflet map to that day's cluster on tap, via a `ref` on `MapContainer` — initially rendered directly behind the floating bottom nav (both at the same bottom offset, nav's z-index way higher) until repositioned above it. Map tab, then auth, remains the stated roadmap — this is a first pass, not the whole thing.
- **Real flag icons instead of emoji, with overlapping badges for multi-country trips** (9 Jul, David sent a screenshot of the TripPicker dropdown: "tidy up the icons... could we merge or blend the country flag when there's more than one"): the actual bug was Scotland — stored as a plain black flag emoji placeholder (no standard Scotland emoji exists), which renders as a broken/tofu glyph on some Android builds, exactly what David saw. New `src/components/CountryFlags.jsx` renders real flag SVGs instead, two-or-more overlapping into a rounded badge stack rather than floating as separate emoji with default text spacing. `src/lib/flags.js`'s `emojiFlagToIso()` decodes any standard flag emoji back to its ISO code via the regional-indicator-character math they're built from (no per-country lookup table needed — `🇭🇰` = regional indicator H + K = "hk"), with an explicit special case for the Scotland placeholder → `gb-sct`. SVGs are self-hosted in `public/flags/` — just the ~12 countries actually used, copied once from the `flag-icons` npm package (not a runtime dependency) — importing that package's full CSS pulled in `background-image` references for all ~250 countries, which `vite-plugin-pwa`'s precache then swept up as real assets and ballooned the bundle by ~400KB before this was caught and fixed. Replaced every raw `countries.join(' ')` emoji render across the whole app (World cards, Photos hero/album, Costs/Journal/Flights section headers, TripPicker, ShareView) — add a new country later by dropping its `1x1/<code>.svg` in from the flag-icons package (or any source) as `public/flags/<code>.svg`.

- **Backlogged: make the sign-in email open the app rather than a browser** (8 Aug, David: "Add to backlog"): tapping "tap here to sign in" in the OTP email opens Chrome/Safari, where Supabase mints a session the Capacitor WebView cannot see — different origin, different storage — so the person is signed in on the web and still signed out in the app. Nothing is wired for this today: `AndroidManifest.xml` carries only the `MAIN`/`LAUNCHER` filter (no `VIEW` filter for `https`), there is no `public/.well-known/assetlinks.json`, no iOS associated-domains entitlement, and nothing in `src/` listens for `appUrlOpen`. Two routes, both needing a rebuild: **App Links / Universal Links** — `assetlinks.json` carrying the Play App Signing SHA-256 (Play Console → App integrity), an `autoVerify` intent filter on `pond.eend.app/auth/*`, the iOS `apple-app-site-association` equivalent, the deep link added to Supabase's redirect allowlist, and an in-app handler taking the token out of the incoming URL to call `verifyOtp`; or the cheaper **custom scheme** — `redirect_to` → `app.eend.pond://auth`, scheme registered in the manifest, same in-app handler, at the cost of the browser flashing open before bouncing across. Until then the six-digit code is the only path that works everywhere, so the email should lead with it and offer the link as the desktop fallback. Not built — explicitly backlog.
- **Backlogged idea: let a signed-in user change their login email** (14 Jul, David): Account tab currently only supports signing in with an email via OTP — there's no way to change the email address tied to an existing account once signed in. Would need `supabase.auth.updateUser({ email })`, which sends a confirmation to the new address before the change takes effect (and possibly a re-confirmation to the old one depending on Supabase's email-change settings) — same custom-SMTP/sender-name setup already in place for magic-link/OTP mail would need to cover this template too. Not built — explicitly backlog, not build now.
